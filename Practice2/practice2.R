...
gong<- read_xlsx('gb_gong.xlsx')
sil<- read_xlsx('gb_sil.xlsx')
...
server <- function(input, output) {
  dtt<- eventReactive(input$go,{
    ...
    dt3<- arrange(dt3,price)
    dt3$price<- as.integer(dt3$price)
    return(dt3)
  })
  output$bar1<- renderAmCharts({
    amBarplot(x = "newaddress", y = "price", data = dtt(), show_values = TRUE, labelRotation = -90)
  })
}
